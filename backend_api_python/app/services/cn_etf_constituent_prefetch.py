    ...
    if force:
        ...
        for code in unique:
            ...
            if isinstance(db_hit, dict) and metrics._snapshot_is_complete(db_hit):
                ...
                cached_hits += 1
                continue
            # Seed partial rows so live gap-fill can reuse net_profit etc.
            if isinstance(hit, dict) and hit:
                seeded = dict(hit)
                if isinstance(db_hit, dict):
                    for key, value in db_hit.items():
    ...
    if pending:
        with ThreadPoolExecutor(max_workers=workers_n) as pool:
            ...
            for fut in as_completed(futures):
                ...
                try:
                    snap = fut.result() or {}
                    if metrics._snapshot_is_complete(snap) or metrics._snapshot_is_usable(snap):
                        warmed += 1
                        if persist and metrics._snapshot_is_usable(snap):
                            persisted += 1
                    else:
                        failed += 1
                except Exception as exc:
