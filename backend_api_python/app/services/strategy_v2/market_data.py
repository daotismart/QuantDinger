...
upstream klines via ``DataSourceFactory``.
"""
...
def _db_market_type_candidates(market_type: Optional[str], market: str) -> List[str]:
    ...
    if str(market) in {"CNFuturesOptions", "CNIndexOptions"}:
    ...
    *,
    ...
    if _prefer_db_bars():
        try:
            ...
            if _coverage_ok(db_frame, start_utc=start_utc, end_utc=end_utc, limit=limit):
