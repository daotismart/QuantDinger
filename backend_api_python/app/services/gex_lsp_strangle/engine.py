...
class ShortStrangleBacktestConfig:
    ...
    # Kelly p from BS short-leg OTM probs (credit_weighted | average | both_otm).
    kelly_win_prob_mode: str = "credit_weighted"
    kelly_rate: float = 0.0
    kelly_vol_floor: float = 0.05
    # Fallback only when BS inputs are missing.
...
def _realized_vol(und: pd.DataFrame, asof: pd.Timestamp, lookback: int = 20) -> float | None:
    """Annualized close-to-close realized vol as BS sigma fallback."""
    if und is None or und.empty or "close" not in und.columns:
        return None
    hist = und.loc[:asof, "close"].astype(float).dropna().tail(max(int(lookback), 5) + 1)
    if len(hist) < 6:
        return None
...
def run_short_strangle_backtest(
    ...
    *,
    ...
    for dt in dates:
        ...
        win_prob_detail: dict[str, Any] = {"source": "prior_fallback"}
        win_prob = estimate_win_prob(
            closed_pnls,
            prior_p=cfg.kelly_prior_win_prob,
            prior_strength=cfg.kelly_prior_strength,
        )
        # Prefer BS short-leg OTM probabilities when a candidate strangle exists.
        if pick is not None:
            call_k = float(pick.get("call_strike") or pick.get("call_wall") or spot)
            put_k = float(pick.get("put_strike") or pick.get("put_wall") or spot)
            dte = walls.get("dte")
            if dte is None and pick.get("expire_date"):
                try:
                    dte = (pd.Timestamp(pick["expire_date"]).normalize() - pd.Timestamp(dt).normalize()).days
                except Exception:
                    dte = None
            t_years = max(float(dte or 0), 1.0) / 365.0
            vol = None
            try:
                if dt in atm_iv.index:
                    vol = float(atm_iv.loc[dt])
            except Exception:
                vol = None
            if vol is None or not (vol == vol) or vol <= 0:
                vol = _realized_vol(und, pd.Timestamp(dt), lookback=max(int(cfg.iv_lookback), 20) // 3)
            if vol is not None:
                vol = max(float(vol), float(cfg.kelly_vol_floor))
            bs = None
            if vol is not None and call_k > 0 and put_k > 0:
                bs = estimate_win_prob_from_bs_legs(
                    spot=spot,
                    call_strike=call_k,
                    put_strike=put_k,
                    t_years=t_years,
                    vol=float(vol),
                    call_premium=float(pick.get("call_close") or 0.0),
                    put_premium=float(pick.get("put_close") or 0.0),
                    rate=float(cfg.kelly_rate),
                    mode=str(cfg.kelly_win_prob_mode or "credit_weighted"),
                )
            if bs is not None:
                win_prob = float(bs["win_prob"])
                win_prob_detail = {
                    "source": "bs_legs",
                    "mode": bs.get("mode"),
                    "callWinProb": round(float(bs["call_win_prob"]), 6),
                    "putWinProb": round(float(bs["put_win_prob"]), 6),
                    "bothOtmProb": round(float(bs["both_otm_prob"]), 6),
                    "vol": round(float(bs["vol"]), 6),
                    "tYears": round(float(bs["t_years"]), 6),
                }
        base_lots = max(int(cfg.lots), 1)
        kelly_info: dict[str, Any] = {
            "winProb": round(win_prob, 6),
            "winProbDetail": win_prob_detail,
            "fraction": None,
            "baseLots": base_lots,
