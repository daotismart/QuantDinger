...
按交易所：CFFEX 8、SHFE 17、DCE 19、CZCE 17、INE 5、GFEX 3。最短日线序列为广期所 `PS0`（397 根，2024-12-26 起）。

### 3.1 分钟线补全（同日）

对全部 69 个主力连续拉取拼接 `1m`（邻近 12 个月），并本地重采样 `5m` / `15m` / `30m` / `1H`。未注册分钟 watch，避免历史维护打爆新浪。首轮后 22 个品种因限流失败，冷却后重试 **21/21 + SC0 成功**，最终 **69/69**。

| 周期 | 品种数 | K 线数 | 覆盖区间 |
| --- | ---: | ---: | --- |
| 1m | 69 | 949,684 | 2023-02-17 → 2026-08-17 |
| 5m | 69 | 265,811 | 2023-02-17 → 2026-08-17 |
| 15m | 69 | 111,179 | 2023-02-17 → 2026-08-17 |
| 30m | 69 | 65,535 | 2023-02-17 → 2026-08-17 |
| 1H | 69 | 41,659 | 2023-02-17 → 2026-08-17 |

夜盘品种最长约 15,345 根 1 分钟线；无夜盘品种（玉米、苹果等）约 8,184 根。新浪单合约窗口约 1023 根，深度来自交割月拼接，不是 2005 年起的完整分钟史。
...
- 生产已入库全部 69 个主力连续的日线、周线，以及拼接后的 1m 与派生 5m/15m/30m/1H（见第 3 节）
