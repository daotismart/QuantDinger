...
def _futures_zh_daily_last(symbol: str) -> Optional[Dict[str, Any]]:
    """Last bar from Sina daily futures (works for CFFEX IF/IH/IM when spot API breaks)."""
    ...
    for query in queries:
        try:
            frame = ak.futures_zh_daily_sina(symbol=query)
            if frame is None or getattr(frame, "empty", True):
                continue
            row = frame.iloc[-1]
            ...
            return {
                ...
                "avg_price": 0.0,
                "prev_close": price,
                "prev_settle": _safe_float(row.get("settle")),
                "source": "futures_zh_daily_sina",
            }
        except Exception as exc:
            logger.debug("futures_zh_daily_sina %s failed: %s", query, exc)
    return None
...
def _futures_zh_spot(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    # CFFEX continuous (IF0/IH0/IM0) and some month contracts currently break
    # futures_zh_spot (akshare column mismatch). Fall back to daily close.
    return _futures_zh_daily_last(code)
