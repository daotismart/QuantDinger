...
def chart_history():
    ...
    try:
        ...
        # Futures options playback (underlying bars + live OI structure)
        if not _is_etf_scope() and chart_key in {"options.gex", "options.gexDist", "gex"}:
            from app.services.cn_derivatives_futures_options_history import (
                build_futures_options_gex_playback,
            )

            data = build_futures_options_gex_playback(
                root,
                interval=interval,
                bars=bars_i if bars_i is not None else 60,
                month=month,
        ...
        if not _is_etf_scope() and chart_key in {
            "options.capital",
            "options.premiumMargin",
        }:
            from app.services.cn_derivatives_futures_options_history import (
                build_futures_options_capital_history,
            )

            data = build_futures_options_capital_history(
                root,
                chart_key=chart_key,
                interval=interval,
                bars=bars_i if bars_i is not None else 60,
