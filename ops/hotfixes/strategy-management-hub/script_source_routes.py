...
@strategy_blp.route("/strategies/inventory", methods=["GET"])
@login_required
def list_strategy_inventory():
    """Strategy management inventory: attributes + backtest/live summaries."""
    try:
        data = build_strategy_inventory(
            user_id=int(g.user_id),
            keyword=str(request.args.get("keyword") or request.args.get("q") or "").strip(),
            status=str(request.args.get("status") or "").strip(),
            asset_type=str(request.args.get("assetType") or request.args.get("asset_type") or "").strip(),
            visibility=str(request.args.get("visibility") or "").strip(),
            limit=int(request.args.get("limit") or 500),
        )
        return jsonify({"code": 1, "msg": "success", "data": data})
    except Exception as exc:
        logger.error("list_strategy_inventory failed: %s", exc)
        return jsonify({"code": 0, "msg": str(exc), "data": {"count": 0, "items": [], "facets": {}}}), 500
